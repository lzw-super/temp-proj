...
def train_one_epoch(model, dataloader, optimizer, depth_loss_fn, pose_loss_fn, epoch, args, scaler=None):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if batch_idx % args.log_every == 0:
            ...
            print(f"[Epoch {epoch}] Batch {batch_idx}/{len(dataloader)} Loss: {total_loss/num_batches:.4f} Time: {elapsed:.2f}s")
...
def main():
    parser = argparse.ArgumentParser(description="Head-Only Training with Replica")
    ...
    print(f"\n{'='*60}")
    print(f"Head-Only Training (Replica)")
    print(f"{'='*60}")
