...
DEFAULT_STAGE1_CKPT="try_train/checkpoints/lzw_map_stage1_frozen_dinov2_2-20_spatial_nearby_5000iters/checkpoint_final.pt"  # 可改：默认 Stage1 初始化权重
...
OUTPUT_DIR="try_train/checkpoints/lzw_map_stage2_${MODE_SUFFIX}_${VIEWS_START}-${VIEWS_END}_k${K_MIN}-${K_MAX}_${TOTAL_ITERS}iters"  # 可改：输出目录命名
...
conda run -n lingbot-map --no-capture-output python try_train/train_lzw_map_stage2.py \
  --data_root "${DATA_ROOT}" \
